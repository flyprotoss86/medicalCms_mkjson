...
const siteUrl='http://site.bjyihuilian.com/index.php?m=content&c=index&a=lists&catid=1548'
const catsUpdatedUrl = 'http://site.bjyihuilian.com/index.php?m=content&c=index&a=lists&catid=1549&siteid=@siteid@&lastupdatedt=@lastupdatedt@'
const itemsUpdatedUrl = 'http://site.bjyihuilian.com/index.php?m=content&c=index&a=lists&catid=1550&tab=@tab@&lastupdatedt=@lastupdatedt@'
...
const rootDir = '/users/Natalya/Desktop/workhis/medicalCmsTest/data/'
...
async function handelContent(dir, content) {
    var reg = new RegExp("[\"'](http:\/\/[a-z]+\.bjyihuilian\.com/)(.+?)(.jpg|.png)[\"']")
    //var reg = new RegExp("bjyihuilian")
    let match
    while(match = reg.exec(content)){
        // console.log(match[0], match[2])
        content = content.replace(match[0], `"${match[2]}${match[3]}"`)
...
async function initJsonData() {
    ...
    for(let i = 0;i < sites.length;i++){
        ...
        try{
            ...
            for(let cIndex = 0; cIndex < cats.length; cIndex++) {
                let c = cats[cIndex]
                let cnt = await handelContent(path.join(rootDir, site.dirname), c.content)
                c.content = cnt
            }
        ...
        for(let j = 0; j < cats.length ; j++) {
            ...
            if(cat.catid == 1128){
                ...
                for(let k=0; k<items.length ; k++) {
                    ...
                    try{
                        ...
                        let cnt = await handelContent(path.join(rootDir, site.dirname), itemDetail.content)
                        itemDetail.content = cnt
            ...
            cats[j].items = items
            // catsFull.push( Object.assign(cats[j]), {'items': itemsFull})
        }

        try{
            fs.writeFile(
                path.join(rootDir, site.dirname, 'datafull.json'),
                JSON.stringify(cats),
                (err) => {
                    if (err){
                        console.log(err)
...
async function Test() {
    let content= "<img alt=\"\" src=\"http://site.bjyihuilian.com/uploadfile/2018/0607/20180607115313615.jpg\" style=\"width: 992px; height: 1403px;\" />\n" +
        "<img alt=\"\" src=\"http://site.bjyihuilian.com/uploadfile/2018/0523/20180523035038363.jpg\" style=\"width: 1000px; height: 2933px;\" />\n"
    let cnt = await handelContent(path.join(rootDir, 'xxyiyuan'),content)
    console.log(cnt)
}
...
// mkDirSync('/users/Natalya/Desktop/workhis/medicalCmsTest/data/xxyiyuan/uploadfile/2018/0607/')
...
function getItemDetail(cid,itemid) {
    return new Promise((resolve, reject)=>{
        getJSON(itemDetailUrl.replace('@cid@',cid).replace('@id@',itemid)).then(reqData=>{
            ...
            reqData=dataList
            if(reqData.length>0){
                let {id,catid,title,thumb,description,inputtime,updatetime,content,keywords} = reqData[0]
                resolve({id,catid,title,thumb,description,keywords,inputtime,updatetime,content})
            } else {
                reject('no data')
            }
        }, err=>{
